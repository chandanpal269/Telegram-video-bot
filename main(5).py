...
async def add_channel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await message.reply_text("I couldn't access that channel. Add the bot to the channel first, preferably as admin, then try again.")
        return
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Admin command to broadcast text to tracked users."""
    if not await is_admin(update, context): return
    message = update.effective_message
    if message is None or not context.args:
        if message: await message.reply_text("Usage: /broadcast <message>")
        return
    text = " ".join(context.args).strip()
    sent = failed = 0
    for user_id in list(get_users(context).keys()):
        try:
