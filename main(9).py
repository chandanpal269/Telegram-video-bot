...
async def send_configured_video(
    ...
    try:
        ...
        # Keep the video message available for 10 minutes, then remove it.
        # Telegram's protected-content flag also disables normal forwarding/saving.
        if sent_message is not None:
            async def delete_expired_video() -> None:
                await asyncio.sleep(VIDEO_VALIDITY_SECONDS)
                try:
                    await sent_message.delete()
                except Exception:
                    LOGGER.exception("Failed to delete expired video message")

            asyncio.create_task(delete_expired_video())
        return True
    except Exception:
